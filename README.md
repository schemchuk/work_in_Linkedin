...
### 9. Серія постів про одну тему

Якщо якийсь тижневий пост добре зайшов аудиторії, можна перетворити його на серію: наступні тижні агент писатиме продовження про той самий проєкт замість вибору нового. Працює через `series_manager.py` (стан у `series_state.json`, гітігнорений).

Як почати:
- **Кнопка після публікації.** Якщо цього тижня не було активної серії, після публікації прийде повідомлення «Тема заходить? Можеш перетворити її на серію» з кнопкою.
- **Команда `/series`** — показує поточну серію (прогрес, кнопка зупинки) або пропонує почати нову з одного з останніх опублікованих проєктів. Обираєш проєкт → скільки частин (2-5 або ♾️ без ліміту).

Поки серія активна:
- Щосереди агент пише **наступну частину** про той самий проєкт замість звичайного вибору. Claude бачить тексти всіх попередніх частин і отримує жорстку інструкцію не повторюватись — кожна частина розкриває новий кут.
- Нові коміти за цей проєкт необов'язкові — якщо їх нема, пост будується на README/описі й тому, що вже було сказано.
- Якщо Claude вирішить, що додати справді нічого — він поверне `skip`, серія автоматично завершується повідомленням, і тиждень переходить до звичайного вибору проєкту.
- Досягнення заданої кількості частин теж завершує серію автоматично.
- Зупинити вручну в будь-який момент: `/series` → «⏹ Зупинити серію».

Чернетка й підтвердження публікації позначені міткою `🔥 Серія — частина N/M`.

### 10. Аналітика: хто клікнув, хто форкнув, хто зіркнув

**LinkedIn не передає referrer на зовнішні сайти** — перевірено на реальних даних: опублікований пост дав GitHub-репозиторію перегляди, але в статистиці referrer стоїть `github.com`, а не `linkedin.com`. Тобто GitHub нативно не може сказати, скільки переглядів прийшло саме з конкретного поста. LinkedIn також ніде — навіть у власному інтерфейсі автора — не показує особу того, хто перейшов за посиланням; це не обмеження API, а принципова відсутність такої функції на боці LinkedIn.

Тому статистика працює трьома незалежними шляхами:

**Точні кліки по кожному посту — трековані посилання (`link_tracker.py`).** Замість прямого `github.com/...` у пост вставляється короткий лінк через [short.io](https://short.io), окремий для німецької та української версії — це і дає точну кількість кліків по конкретному посту конкретною мовою. Потребує зовнішнього акаунту (безкоштовний тариф short.io: 1000 лінків/міс):
1. Зареєструйся на https://short.io
2. Підтверди домен у дашборді (безкоштовний піддомен `xxxxx.short.gy`, який дають одразу, підходить)
3. Створи API-ключ: Settings → Integrations & API
4. Впиши `SHORTIO_API_KEY` і `SHORTIO_DOMAIN` у `.env`
5. Перевір налаштування: `python link_tracker.py`

Без цих змінних усе просто працює як раніше — пости йдуть зі звичайним `github.com`-посиланням, без трекінгу (жодних збоїв).

**Форки/зірки — реальні GitHub-акаунти (`repo_watch.py`).** Бот щогодини перевіряє кількість форків/зірок по всіх публічних репо (один дешевий запит на всі), і якщо число зросло — підтягує список акаунтів і надсилає в Telegram, хто саме форкнув чи зіркнув, з посиланням на профіль. При першому запуску існуючі форки/зірки просто беруться за базову лінію без сповіщень (щоб не було зливи повідомлень про історичні дані).

**Трафік GitHub-репозиторію — щоденні знімки (`traffic_snapshot.py`).** GitHub зберігає власну статистику (views/clones/referrers) лише за останні 14 днів, тому таймер `traffic-snapshot.timer` щодня о 08:00 знімає поточні цифри для всіх репозиторіїв, які колись згадувались у пості з посиланням, і накопичує в `traffic_history.json`. Той самий запуск оновлює лічильники кліків трекованих посилань.

```bash
cp systemd/traffic-snapshot.{service,timer} ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now traffic-snapshot.timer
```

Все зведено в `/stats` у Telegram-боті: реакції/коментарі з email, кліки по трекованих посиланнях, останній знімок трафіку GitHub, і поточні форки/зірки.

### 11. Перевірка репозиторію перед публікацією посилання
