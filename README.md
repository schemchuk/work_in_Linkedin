...
**Нема LICENSE — виправляється автоматично.** Це єдина перевірка, яку можна полагодити без жодних рішень: `ensure_license()` клонує репозиторій, додає стандартну MIT-ліцензію, комітить і пушить у origin твоїм git-ідентити (`git config user.name/email`) і SSH-ключем — без підтвердження, бо додати дозвільну ліцензію у власний репозиторій безпечно й легко відкотити. Вимикається через `AUTO_FIX_LICENSE=false` у `.env`.
...
**Витік секретів — окреме, жорсткіше правило.** Якщо gitleaks знаходить секрет будь-де в історії репозиторію (не лише README/LICENSE — це вже критично), то:
1. посилання на нього блокується назавжди, без винятків;
2. у Telegram і на email летить сповіщення з деталями: скільки знахідок, у яких файлах, які типи секретів, за який період — без самих значень секретів (gitleaks працює з `--redact`).

Сповіщення надсилається один раз на кожен HEAD-коміт репозиторію (щоб щотижневий запуск агента не спамив про той самий витік), і повторюється знову, якщо в репозиторій додасться новий коміт з новим витоком. Видалення файлу наступним комітом не рахується — секрет лишається в історії, тому єдиний реальний fix — відкликати/перегенерувати самі облікові дані.

**Багатошаровий захист від витоків.** `repo_safety.py` перевіряє репозиторій лише тоді, коли він стає кандидатом на пост — цього недостатньо, якщо секрет потрапить у репозиторій, про який агент цього тижня не пише. Тому є ще два шари:

1. **Локальний pre-commit hook (`git-hooks/pre-commit`) — блокує коміт ще до того, як секрет потрапить у git.** Це найдешевша точка захисту: секрет, якого немає в жодному коміті, не можна ні знайти в історії, ні опублікувати. Hook встановлюється глобально (`core.hooksPath`) і працює для **будь-якого** git-репозиторію на цій машині, не лише публічних. Встановлення:
   ```bash
   mkdir -p ~/.config/git/hooks
   ln -sf "$(pwd)/git-hooks/pre-commit" ~/.config/git/hooks/pre-commit
   git config --global core.hooksPath ~/.config/git/hooks
   ```
   Хибне спрацювання: `git commit --no-verify` (разово) або виняток у `.gitleaks.toml` конкретного репозиторію (постійно).

2. **Щотижневий повний аудит усіх публічних репозиторіїв** (не лише кандидатів на пост) — `repo-safety-audit.timer`, неділя 09:00. Один прогін `python repo_safety.py` без аргументів проходить по всіх публічних репозиторіях, і якщо десь є витік — надсилає **один зведений** алерт у Telegram + email, повторюючи його щотижня, доки проблему не вирішено (на відміну від алертів під час генерації поста, які не повторюються для того самого коміту).

   ```bash
   cp systemd/repo-safety-audit.{service,timer} ~/.config/systemd/user/
   systemctl --user daemon-reload
   systemctl --user enable --now repo-safety-audit.timer
   ```
