...
- reply_draft: ЯКЩО kind=comment — чернетка відповіді НІМЕЦЬКОЮ:
  1-3 речення, тепло і по суті, подякуй і додай змістовну думку або
  зустрічне запитання; без хештегів, без формальностей типу
  "Sehr geehrte"; звертайся на "du", якщо тон коментаря неформальний,
  інакше "Sie". Якщо kind не comment — "".
